...
import FollowsDisplay from "./_components/follows-display";
...
async function getUserProfile({ params }: { params: { id: string } }) {
  ...
  const apiRoute =
    ...
    (hasParams ? "/api/users/handle/" : "/api/users/");
...
async function getFollowData(userId: string) {
  const followData = await fetch(
    process.env.NEXT_PUBLIC_BASE_URL + "/api/users/follow-data/" + userId,
    {
      next: {
        revalidate: 0,
      },
...
export default async function Profile({
      ...
      <div className={className}>
        <div className="max-w-[2000px] m-auto flex py-8 text-white">
          <Image
            ...
            width="110"
            height="110"
            className="border border-border shadow-md rounded-sm"
          />

          <div className="ml-5 flex flex-col">
            <div className="flex mt-1 items-baseline">
              <p className="font-extralight text-3xl tracking-tight">
                {firstName + " " + lastName}
              </p>
              ...
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      <FollowsDisplay option="followers" userId={id} />
                    </DialogDescription>
                  </DialogHeader>
                </DialogContent>
              </Dialog>
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      <FollowsDisplay option="following" userId={id} />
                    </DialogDescription>
                  </DialogHeader>
                </DialogContent>
              </Dialog>
    ...
    return (
      <Card className="border-border shadow-md">
        <CardHeader>
          ...
          <CardDescription className="text-md whitespace-pre-line">
            {aboutMe ? aboutMe : "Nothing here... yet!"}
          </CardDescription>
        </CardHeader>
        {emailPublic && (
          <CardContent>
            <CardDescription className="text-md flex gap-2">
              Email
            </CardDescription>
              ...
              {(externalUrls as []).map(({ url }: { url: string }) => (
                <li key={url}>
                  <a
                    ...
                    className="align-middle underline hover:text-primary"
                  >
                    {url}
                  </a>
                </li>
  ...
  async function LikesCard() {
    const likes: [Likes] = await fetch(
      process.env.NEXT_PUBLIC_BASE_URL + "/api/models/like-data/" + user.id,
      {
        next: { revalidate: 0 },
      }
    ).then((res) => res.json());
    ...
    for (let i = 0; i < likes.length; i++) {
      await fetch(
        process.env.NEXT_PUBLIC_BASE_URL + "/api/models/" + likes[i].modelId,
        {
          next: { revalidate: 0 },
        }
      ).then(async (res) => {
  ...
  async function ModelsCard() {
    const results = await fetch(
      process.env.NEXT_PUBLIC_BASE_URL + "/api/models/user/" + user.id,
      {
        next: {
          revalidate: 0,
        },
